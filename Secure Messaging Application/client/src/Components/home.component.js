...
const Home = ({location}) => {
    ...
    const verifySignature = (msg) => {
        try {
            const publicKey = forge.pki.publicKeyFromPem(msg.publicKey);
            const md = forge.md.sha256.create();

            // Use the encrypted message string (same as backend signed)
            md.update(msg.message, 'utf8');

            const signatureBytes = forge.util.hexToBytes(msg.signature);
                                ...
                                return <div className="messageContainer sentMessage" key = {message.iv}>
                                    <div className="messageBox messageBox-sent">
                                        <p className="messageText">
                                             {decryptMessage(message.key, message.message, message.iv)}
                                                <br />
                                                <span style={{ fontSize: "10px", color: verifySignature(message) ? "limegreen" : "red" }}>
                                                    Signature: {verifySignature(message) ? " Valid" : " Invalid"}
                                                </span>
                                        </p>
                                ...
                                return <div className="messageContainer receivedMessage" key = {message.iv}>
                                    <div className="messageBox messageBox-receive">
                                        <div className="messageText">
                                            ...
                                            :null}
                                            <p>{decryptMessage(message.key, message.message, message.iv)}
                                                <br/>
                                                <span style={{ fontSize: "10px", color: verifySignature(message) ? "limegreen" : "red" }}>
                                                    Signature: {verifySignature(message) ? "Valid" : "Invalid"}
                                                </span></p>
